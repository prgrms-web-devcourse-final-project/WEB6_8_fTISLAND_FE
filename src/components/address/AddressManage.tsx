...
export default function AddressManage({
  ...
  const handleFindCurrentLocation = React.useCallback(() => {
        ...
        if (window.kakao?.maps?.services) {
          ...
          geocoder.coord2Address(longitude, latitude, (result: any, status: any) => {
            if (status === window.kakao?.maps?.services.Status.OK && result?.[0]) {
              ...
              if (addr) {
                setValue('selectedAddress', { address: addr });
                setPreviewMap(addr);
              }
            }
          });
        } else {
          const approx = `위도 ${latitude.toFixed(4)}, 경도 ${longitude.toFixed(4)}`;
          setValue('selectedAddress', { address: approx });
          setPreviewMap(approx);
        }
      },
      (err) => {
  ...
  const homeAddr = React.useMemo(() => localSavedAddresses.find((a) => a.type === 'home'), [localSavedAddresses]);
  const companyAddr = React.useMemo(() => localSavedAddresses.find((a) => a.type === 'company'), [localSavedAddresses]);
  ...
  const card = (
    <Card
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={`flex ${asDialog ? 'h-full flex-col' : ''}`}>
        <CardContent className={`space-y-5 px-6 ${asDialog ? 'flex-1 overflow-y-auto' : ''}`}>
          <section className='space-y-3'>
            ...
            <div className='flex items-center gap-2 rounded-2xl border border-[#bbe7e4] bg-[#f0fffd] px-3 py-2.5'>
              ...
              <Button
                ...
                size='sm'
                variant='outline'
                className='h-8 rounded-full border-[#2ac1bc]/50 px-3 text-[12px] font-semibold text-[#2ac1bc] hover:bg-[#2ac1bc]/10'
